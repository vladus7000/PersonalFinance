...
| [ARCHITECTURE.md](ARCHITECTURE.md) | Карта слоёв кода («куда класть новый файл»), таблица-памятка по тестам, рецепт «как добавить экран с нуля» |
...
отметки ✅ в разделе «Прогресс» ниже и следуйте её Definition of Done. Эпики
E0–E2 завершены — начинать с **E3**.
...
- [x] **E2** — Профиль + онбординг: таблица/репозиторий UserProfile, движок шагов (step engine), шаги «валюта» и «жизнь», экран первого запуска, роутинг с редиректом на онбординг
...
Строки — в `lib/core/l10n/app_<locale>.arb` (по файлу на язык, `en` — базовый/шаблонный).
Новые строки добавлять только туда, никогда не хардкодить текст в виджетах — см.
[ARCHITECTURE.md](ARCHITECTURE.md). После правки ARB-файлов: `flutter gen-l10n`.

Если перевод для какого-то ключа отсутствует, `gen-l10n` автоматически подставит текст из `en`
(проверено) и молча не сломает сборку — но покажет разрыв в `lib/core/l10n/untranslated.json`
(не коммитится, перегенерируется каждый раз):

```bash
flutter gen-l10n
cat lib/core/l10n/untranslated.json   # {} значит переведено всё
```

**Добавить новый язык:** `dart run tool/scaffold_locale.dart <код>` создаёт готовый к переводу
файл — новый язык подключается автоматически, без правок кода (проверено). Что делать дальше и
что отправить переводчику — [lib/core/l10n/TRANSLATING.md](lib/core/l10n/TRANSLATING.md), это
письмо можно пересылать переводчику как есть, без технического контекста.
